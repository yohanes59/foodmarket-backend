                    ...
                    <tbody>
                        @forelse ($transaction as $item)
                            <tr>
                                ...
                                <td class="border px-6 px-4">{{ $item->food->name }}</td>
                                <td class="border px-6 px-4">{{ $item->user->name }}</td>
                                <td class="border px-6 px-4">{{ $item->quantity }}</td>
                                <td class="border px-6 px-4">{{ number_format($item->total) }}</td>
                                <td class="border px-6 px-4">{{ $item->status }}</td>
                                <td class="border px-6 px-4 text-center">
                                    <a href="{{ route('transaction.show', $item->id) }}"
                                        class="inline-block bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 mx-2 rounded">
                                        View
                                    </a>
